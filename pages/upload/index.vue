    ...
    <div class="w-full mt-[80px] mb-[40px] bg-white shadow-lg rounded-md py-6 md:px-10 px-4">
      ...
      <div class="mt-8 md:flex gap-6">
        <label v-if="!fileDisplay" for="fileInput" @drop.prevent="onDrop" @dragover.prevent=""
          ...
          <Icon name="majesticons:cloud-upload" size="40" color="#b3b3b1"></Icon>
          <div class="mt-4 text-[17px]">Select video upload</div>
          ...
          <div class="px-2 py-1.5 mt-8 text-white text-[15px] w-[80%] bg-[#F02C56] rounded-none">Select file</div>
          <input ref="file" type="file" id="fileInput" @input="onChange" hidden accept=".mp4">

        </label>
        <div v-else
          class=" md:mx-0 mx-auto mt-4 md:mb-12 mb-6 flex items-center justify-center w-full max-w-[260px] h-[540px] p-3 rounded-2xl cursor-pointer relative">
          <div class="bg-black h-full w-full"> </div>
          <img class=" absolute z-20 pointer-events-none" src="~/assets/images/mobile-case.png">
          <img class=" absolute right-4 bottom-4 z-20" width="90" src="~/assets/images/tiktok-logo-white.png">
          <video autoplay loop muted class=" absolute rounded-xl object-cover mx-auto h-full  w-full p-[13px] z-10"
        ...
        <div class=" mt-4 mb-6">
          <div class="flex bg-[#f8f8f8] py-4 px-6">
            ...
            <div>
              <div class="text-semibold text-[15px] mb-1.5">Divide videos edit</div>
              <div class="text-semibold text-[13px] text-gray-400">You can quickly divide videos into multiple parts,
                remove redundant parts and turn landscape videos into portrait videos</div>
            </div>
